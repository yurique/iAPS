...
struct SelectedFoodView: View {
    ...
    private var displayCarbs: Decimal {
        isAIProduct ? food.carbs : food.carbs * (Decimal(portionGrams) / 100.0)
    }

    private var displayFat: Decimal {
        isAIProduct ? food.fat : food.fat * (Decimal(portionGrams) / 100.0)
    }

    private var displayProtein: Decimal {
        isAIProduct ? food.protein : food.protein * (Decimal(portionGrams) / 100.0)
    }

    private var displayCalories: Decimal {
        isAIProduct ? food.calories : food.calories * (Decimal(portionGrams) / 100.0)
    }
